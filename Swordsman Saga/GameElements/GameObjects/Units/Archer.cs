...
class Archer : IRanged
{
    ...
    public Texture2D TextureBeingAttacked { get; set; }
    ...
    */
    ...
    public Archer(string id, int x, int y, int player, DynamicContentManager contentManager, FightManager fightManager, StatisticsManager statisticsManager)
    {
        if (id == null)
        {
        ...
        Position = new Vector2(x, y); //TODO Question (by Marvin): Why is the position not part of the grid? Or is it?
                                      //The Position isn't part of a grid. It can be anywhere. This unit is stored in a grid square though.
        ...
        Attacked = true;
        TextureIsSelected = contentManager.Load<Texture2D>("2DAssets/GameObjectAssets/Units/friendly_archer_is_selected");
        Texture = contentManager.Load<Texture2D>("2DAssets/GameObjectAssets/Units/" + (player == 0 ? "friendly" : "enemy") + "_archer");
    }
